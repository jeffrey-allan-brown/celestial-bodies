...
const validationSchema = Joi.object({
  ...
  end_date: Joi.date(),
  key: Joi.string(),
  start_date: Joi.date().less(Joi.ref('endDate')),
  thumbs: Joi.boolean()
});
...
async function apod(payload, callback) {
  ...
  if (payload.date && (payload.startDate || payload.endDate)) throw new Error('If using Date, you cannot also use Start Date or End Date');
  if (payload.endDate && !payload.startDate) throw new Error('Start Date must be included if using End Date');
  if (payload.count && (payload.startDate || payload.endDate)) throw new Error('If using count, you cannot use Start Date or End Date');
